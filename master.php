...
<body class="<?php print(app_body_classes()); ?>">
    <div class="adm-page">
        <div class="<?php print(app_page_shell_classes()); ?>">
            <div class="adm-header <?php print(app_header_classes('mb-4')); ?>">
            ...
            <div
                id="masterStatusMessage"
                class="adm-alert success <?php print(app_alert_classes('success')); ?><?php if ($message === '') {
                                                                                            print(' hidden');
                                                                                        } ?>"
                role="alert"><?php print(h($message)); ?></div>
            ...
            <section class="adm-panel mb-4 <?php print(app_panel_classes()); ?>">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div class="message-visible-cell">
                        <form method="post" action="master.php?s=<?php print((int)$station_id); ?>" data-language-toggle-form data-ajax-skip="1">
                            <input type="hidden" name="station_id" value="<?php print((int)$station_id); ?>">
                            <button
                                ...
                                <span
                                    class="message-toggle-switch <?php print(app_toggle_switch_classes($display_english_enabled)); ?>"
                                    data-language-toggle-switch
                                    ...
                                    aria-hidden="true">
                                    <span
                                        class="block <?php print(app_toggle_knob_classes($display_english_enabled)); ?>"
                                        data-language-toggle-knob
                                        data-class-on="<?php print(h('block ' . app_toggle_knob_classes(true))); ?>"
                                        data-class-off="<?php print(h('block ' . app_toggle_knob_classes(false))); ?>"></span>
                                </span>
                                <span class="message-toggle-label text-[11px]" data-language-toggle-label><?php print($display_english_enabled ? 'ON' : 'OFF'); ?></span>
                            </button>
                        </form>
                    </div>
                </div>
                    ...
                    <div>
                        <section class="adm-card h-full <?php print(app_panel_classes('flex h-full flex-col gap-5')); ?>">
                            ...
                            <div class="adm-card-actions mt-auto">
                                <a class="adm-link-btn<?php if ($card['accent']) {
                                                            print(' accent');
                                                        } ?> <?php print(app_button_classes($card['accent'] ? 'primary' : 'secondary')); ?>" href="<?php print(h($card['href'])); ?>">開く</a>
                            </div>
                        </section>
                    </div>
                <?php } ?>
    ...
    <?php render_app_scripts(array('js/master.js?v=1.0.0')); ?>
</body>
