...
<body class="<?php print(app_body_classes()); ?>">
    <div class="adm-page">
        <div class="<?php print(app_page_shell_classes()); ?>">
            ...
            <section class="adm-panel content-setting-panel <?php print(app_panel_classes()); ?>">
                ...
                <form id="publishForm" class="mt-6 space-y-6" method="post" action="content_setting.php" data-ajax-skip="1">
                    ...
                    <div class="content-dropzone rounded-[28px] border border-dashed border-slate-300 bg-slate-50 px-5 py-5" id="contentDropzone">
                        ...
                        <div class="content-dropzone-empty mt-4 rounded-[22px] border border-dashed border-slate-300 bg-white px-5 py-4 text-center text-sm text-slate-500" id="dropzoneEmpty">下のコンテンツを3つの表示枠へドラッグ&ドロップしてください。</div>
                        <div class="content-selected-grid mt-4 grid gap-4 lg:grid-cols-3" id="selectedList" aria-live="polite"></div>
                    </div>
                            ...
                            <?php foreach ($setting_items as $item) { ?>
                                <div class="content-gallery-item">
                                    <button type="button" class="content-gallery-card js-setting-item group w-full overflow-hidden rounded-[28px] border border-slate-200 bg-white text-left shadow-[0_12px_30px_rgba(15,23,42,0.06)] transition hover:-translate-y-0.5 hover:shadow-[0_18px_35px_rgba(15,23,42,0.08)]" draggable="true" data-id="<?php print((int)$item['id']); ?>">
                                        ...
                                        <div
                                            class="content-card-thumb<?php if ($item['type'] === 'movie') {
    ...
    <script id="content-setting-data" type="application/json">
        <?php print(json_encode(array('items' => $setting_items, 'selected' => array_values($published_ids), 'limit' => CONTENT_DISPLAY_MAX), $json_flags)); ?>
    </script>
    <?php render_app_scripts(array('js/content_preview.js?v=1.0.0', 'js/content_setting.js?v=1.0.2')); ?>
</body>
