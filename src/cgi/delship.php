...
try{
    ...
    while ($row = $stt->fetch()) {
        ...
        //error_log("delship time=".$time.",st1=".$st1.",st2=".$st2.",date=".strftime('%Y-%m-%d %H:%M:%S',time())."\r\n", 3, 'debug.log');
        $sst2 = $db->prepare('UPDATE timetable 
            SET ship_id = 0 WHERE station_id = :station_id AND time = :time AND destination_id = :station1 AND destination2_id = :station2');
