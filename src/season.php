...
if (isset($_SESSION['season_flash']) && is_array($_SESSION['season_flash'])) {
    $status_type = trim((string)($_SESSION['season_flash']['type'] ?? ''));
    $status_message = trim((string)($_SESSION['season_flash']['message'] ?? ''));
    unset($_SESSION['season_flash']);
}
    ...
    try {
        if ($action === 'create' || $action === 'update') {
            ...
            if ($action === 'create') {
                ...
                $_SESSION['season_flash'] = array(
                    'type' => 'success',
                    'message' => 'ダイヤ期間を追加しました。',
                );
                redirect_with_params('season.php', array(
                    's' => $station_id,
                    'd' => $requested_dial,
                ));
            } else {
...
>
    <div class="app-modal-dialog <?php print(app_modal_dialog_classes('md')); ?>">
        <div class="app-modal-card <?php print(app_modal_card_classes()); ?>">
            ...
            <form class="dial-create-form <?php print(app_modal_body_classes('pt-0')); ?>" method="post" action="season.php" data-ajax-skip="1">
            <input type="hidden" name="s" value="<?php print($station_id); ?>">
            <input type="hidden" name="d" value="<?php print(h($requested_dial)); ?>">
