...
function notify_redirect_page(int $station_id, array $flash = array()): void
{
    ...
    redirect_with_params(notify_self_path(), array('s' => $station_id));
}

function notify_self_path(): string
{
    $self = trim((string)($_SERVER['PHP_SELF'] ?? 'notify.php'));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'select') {
            $reset_to_normal = trim((string)($_POST['reset_to_normal'] ?? '')) === '1';
            $selected_key = $reset_to_normal ? '' : trim((string)($_POST['selected_key'] ?? ''));
            $state = notify_load_state();
...
$notify_page_base = notify_self_path();
$notify_page_action = $notify_page_base . '?s=' . $station_id;
    ...
    <div class="adm-page">
        <div class="<?php print(app_page_shell_classes()); ?>">
            ...
            <section class="adm-panel mb-4 <?php print(app_panel_classes()); ?>">
                <form class="adm-toolbar flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between" method="get" action="<?php print(h($notify_page_base)); ?>">
                ...
                <form class="mt-4 space-y-4" method="post" action="<?php print(h($notify_page_action)); ?>" data-ajax-skip="1">
                    ...
                    <div class="<?php print(app_field_classes('max-w-xl')); ?>">
                        ...
                        <select id="selectedNotice" class="<?php print(app_select_classes()); ?>" name="selected_key">
                            <option value="" <?php if ($selected_key === '') {
                                                    print('selected');
                                                } ?>>通常表示</option>
                            <?php foreach ($definitions as $key => $definition) { ?>
                                <?php $has_image = trim((string)($state['items'][$key]['image_path'] ?? '')) !== ''; ?>
                                <option value="<?php print(h($key)); ?>" <?php if ($selected_key === $key) {
                                                                                print('selected');
                                                                            } ?> <?php if (!$has_image) {
                                                                                        print('disabled');
                                                                                    } ?>>
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        <button class="adm-btn <?php print(app_button_classes('secondary')); ?>" type="submit" name="reset_to_normal" value="1">通常表示に戻す</button>
                    </div>
                </form>
            </section>
                ...
                <?php foreach ($notice_cards as $card) { ?>
                    <div>
                        <section class="adm-panel h-full <?php print(app_panel_classes('flex h-full flex-col')); ?>">
                            ...
                            <form class="mt-5 space-y-4" method="post" action="<?php print(h($notify_page_action)); ?>" enctype="multipart/form-data" data-ajax-skip="1">
                                <input type="hidden" name="s" value="<?php print($station_id); ?>">
